...
- Current version: 0.17.0 (keep `manifest.json`, `package.json`, and `versions.json` in sync on every release).
...
Shipped in 0.17.0: private repos now roll into every GitHub stat. With a token set, `src/github.ts` calls the **authenticated** endpoints (`/user`, `/user/repos?visibility=all&affiliation=owner,organization_member`) — `/users/{username}/repos` returns public repos only no matter what token is attached, which is why the old token setting could never surface private work. Each authenticated call falls back to its public equivalent so an expired or under-scoped token degrades instead of blanking the tab. Added `privateRepos` + `authenticated` to `GitHubData`, a PRIVATE readout on GITHUB UPLINK, and an inline hint when running unauthenticated. Settings now spell out the exact token permissions (classic `repo`, or fine-grained Metadata/Contents/Pull requests/Issues/Actions **plus the account-level "Events" read permission** — without Events, private commits stay invisible) and warn that LOG COMMITS writes private commit messages into daily notes.
